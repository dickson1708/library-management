...
public interface ReviewRepository extends JpaRepository<Review, Long>, JpaSpecificationExecutor<Review> {
    ...
    @Query(
        value = "select review from Review review left join fetch review.member left join fetch review.book",
        countQuery = "select count(review) from Review review"
    )
    Page<Review> findAllWithToOneRelationships(Pageable pageable);

    @Query("select review from Review review left join fetch review.member left join fetch review.book")
    List<Review> findAllWithToOneRelationships();

    @Query("select review from Review review left join fetch review.member left join fetch review.book where review.id =:id")
