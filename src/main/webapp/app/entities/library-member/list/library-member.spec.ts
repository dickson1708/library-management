...
import { sampleWithRequiredData } from '../library-member.test-samples';
...
describe('LibraryMember Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: LibraryMember;
  let fixture: ComponentFixture<LibraryMember>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LibraryMember);
  ...
  it('should call load all on init', async () => {
    ...
    expect(comp.libraryMembers()[0]).toEqual(expect.objectContaining({ id: 8289 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 8289 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.libraryMembers()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 8289 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to libraryMemberService', () => {
      ...
      vitest.spyOn(service, 'getLibraryMemberIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLibraryMemberIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.libraryMembersParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.libraryMembersParams()).toMatchObject(
