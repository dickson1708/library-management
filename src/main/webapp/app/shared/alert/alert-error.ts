...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('libraryManagementApp.error', (response: EventWithContent<unknown> | string) => {
    ...
    this.httpErrorListener = this.eventManager.subscribe(
      'libraryManagementApp.httpError',
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`libraryManagementApp.${fieldError.objectName as string}.${convertedField}`);
